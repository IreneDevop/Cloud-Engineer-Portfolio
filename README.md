...
* **Resolution:** Cleared the web directory using `sudo rm -rf *` to ensure a clean deployment environment, followed by cloning the repository with `sudo git clone <repo_url> .` and restarting the web service (`sudo systemctl restart httpd`).
